    ...
    module Flaggable
      ...
      module ClassMethods
        def flags(flag_array=[])
          ...
          scope :flagged, ->(value){joins(join).where("flag_key_values.value IN (#{(value.class!=Array ? [value.to_s] : value).map{|v| "'#{v.to_s.parameterize}'"}.join(',')})").group("#{self.table_name}.id")}
          ...
          flag_array.each do |name|
            ...
            scope :flagged_with, ->(name){joins(join).where('flag_key_values.key=? and flag_key_values.value=?', :flag, name.to_s.parameterize)}
